...
function createRGBAstring(hex_red, hex_green, hex_blue, alpha) {
  if (typeof alpha === "undefined") {
    ...
    rgbaString =
      "rgba(" + hex_red + "," + hex_green + "," + hex_blue + "," + alpha + ")";
  }
  return rgbaString;
}
...
function draw() {
  ...
  ctx.fillStyle = createRGBAstring(
  ...
  for (var y = 0; y < drops.length; y++) {
    ...
    if (root.rainbow) {
      hue += 0.01;
      var hex_red = Math.floor(
        127 * Math.sin(root.rainbowSpeed * hue + 0) + 128,
      );
      var hex_green = Math.floor(
        127 * Math.sin(root.rainbowSpeed * hue + 2) + 128,
      );
      var hex_blue = Math.floor(
        127 * Math.sin(root.rainbowSpeed * hue + 4) + 128,
      );
    } else {
